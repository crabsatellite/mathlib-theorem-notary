...
inputs to further research. Even after a theorem is proved, an independent
consumer must identify its exact statement and assumptions, obtain its proof
dependencies, and establish which checks remain valid through later reuse.
Existing package imports and proof checkers provide essential mechanisms;
successive publications also need a shared contract for carrying these choices
forward. We propose Theorem Notary, a Glass Box protocol for a growing network
of independently published theorem components. Each eligible theorem has a
complete proof checkable by the kernel, the proof assistant's logical core,
under a disclosed foundation. Its certificate binds the full statement,
definitions, source and proof data, and inherited publications. Publicly inspectable
proof material supplies the Glass Box justification; provider signatures record
supply without adding logical authority. Consumers select exact identities,
check every inherited certificate, and reuse only their own authenticated
acceptance for an unchanged checking scope. Derived results enter the same
publication cycle. A Lean reference profile specifies these boundaries and
their build integration. Evaluation combines actual theorem consumption across
independent project boundaries with a large archived formalization and attacks
on statement identity, inner proof obligations, and acceptance reuse. The
protocol makes completed proofs and valid acceptance reusable inputs to new
derivations. It supports cumulative formal knowledge across independently
maintained repositories, providing an admission contract for subsequent human
and automated mathematical development.
\end{abstract}
...
the result for a third group. Each handoff must preserve the precise lemma,
...
Lean already supports reuse through external packages and module imports.
...
Lean's replay utility can validate an environment by sending its declarations
back to the kernel~\citep{leanreplay}. Recording the exact checked objects and
checking scope makes this completed work available to later builds: theorem
proofs can be imported, and a consumer's still-valid acceptance can be reused.
...
\subsection{Glass Box publication and repeated derivation}

Theorem Notary makes each eligible theorem a separately identifiable component.
Its public interface contains the full statement, including hypotheses; its
justification contains inspectable definitions, proof data, and dependencies.
This is the Glass Box relationship: a client uses the theorem as an interface,
while the kernel, the proof assistant's logical core, can check why it holds.
Eligible components are Kernel-Only: their proofs are complete under a
disclosed, permitted foundation, with no extra unproved theorem admitted as an
axiom. Signing supplies no missing proof.
...
applies the same rule to the whole publication chain, including inner
certificates unused by its chosen outer theorem. The output of one accepted
project can thus become an explicit input to another publication cycle.

Provider credit identifies the key supplying a component. Anyone may provide
...
authenticated acceptance for an unchanged checking scope. Registries and
mirrors support discovery and distribution while acceptance remains with the
receiving project. The protocol standardizes these boundaries; its reference
implementation demonstrates a concrete Lean profile and build integration.
...
the objects, admission procedure, composition argument, and Lean profile.
